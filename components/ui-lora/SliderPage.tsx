...
const fragmentShader = `
  ...
  void main() {
    vec4 finalColor;
    if (uEffectType == 0) {
        finalColor = glassEffect(vUv, uProgress);
    } else {
        finalColor = genericEffect(vUv, uProgress); 
    }

    float rnd = noise(gl_FragCoord.xy);
    finalColor.rgb -= (rnd / 15.0) * uNoiseIntensity;
...
const SLIDES = [
  {
    title: "Document Studio Workspace",
    media: "/features/feature-1.jpeg", // 🖼️ Replace with screenshot of /resumes/create form dashboard later
  },
  {
    title: "Realtime Matrix Auditing",
    media: "/features/feature-2.jpeg", // 🖼️ Replace with screenshot of /resumes/analyze score metrics panel later
  },
  {
    title: "Interactive Section Ordering",
    media: "/features/feature-3.jpeg", // 🖼️ Replace with screenshot of Drag & Drop vertical sorting matrix later
  },
  {
    title: "Simulated Spoken Evaluation",
    media: "/features/feature-4.jpeg", // 🖼️ Replace with screenshot of /interviews voice agent room later
  },
  {
    title: "Diagnostic Performance Audit",
    media: "/features/feature-5.jpeg", // 🖼️ Replace with screenshot of /interviews/results assessment view later
  },
];
...
export default function SliderPage() {
  ...
  useEffect(() => {
    ...
    logic.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uNoiseIntensity: { value: SLIDER_CONFIG.settings.noiseIntensity },
    ...
    if (!logic.pane) {
      ...
      f1.addBinding(SLIDER_CONFIG.settings, "noiseIntensity", {
        min: 0.0,
        max: 2.0,
      }).on(
        "change",
        (ev: any) =>
          (logic.material!.uniforms.uNoiseIntensity.value = ev.value),
      );
  ...
  return (
    <main
      ref={containerRef}
      className={`relative w-full rounded-[30px] h-screen overflow-hidden  font-sans transition-opacity duration-1000 ${loaded ? "opacity-100" : "opacity-0"}`}
      onClick={(e) => {
        if (!(e.target as HTMLElement).closest(".slides-navigation"))
          nextSlide();
      }}
    >
      <style jsx global>{`
